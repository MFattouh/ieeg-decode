...
def plot_folds(dataset_dir):
    ...
    subject = os.path.basename(os.path.basename(csv_pathes[0]))
    df_list = []
    ...
    for day in days:
        ...
        sns.factorplot(x='fold', y='corr', hue='exp', kind='bar', size=6, data=gp,
                       order=['fold%d' % fold for fold in range(1, n_folds+1)]
                       , hue_order=['1Layer', '2Layers', '3Layers'])  # needed to order the layers
